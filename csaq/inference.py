"""
csaq/inference.py — Self-Speculative Decoding Engine (v0.2.3)
...
class CSAQInferenceEngine:
    ...
    Usage::
        ...
        output, report = engine.generate(input_ids, max_new_tokens=64)
        output, report = engine.generate(input_ids, speculative=True, lookahead=4)
    """
    ...
    def __init__(
        ...
        # causal_map is keyed by MODULE names (e.g. "model.layers.0.q_proj")
        # NOT parameter names. We resolve each directly.
    ...
    def _resolve_module(self, name: str) -> Optional[nn.Module]:
        """Resolve dotted module name. Strips trailing '.weight' if present."""
        parts = name.split(".")
        if parts[-1] == "weight":
            parts = parts[:-1]
        mod = self.model
    ...
    def generate(
        ...
        **kwargs,
        ...
        ALWAYS returns (output_tensor, SpeculativeReport).
        """
        if speculative:
    ...
    def generate_speculative(
        ...
        If 0 hooks are registered, falls back to standard generation
        and returns a fully-populated report (all metrics = 0.0/safe defaults)
        so audit scripts never crash on None or missing fields.
        """
        # ── Graceful fallback if no hooks ─────────────────────────────────
        if not self._hooks:
            warnings.warn(
        ...
        while tokens_produced < max_new_tokens:
            ...
            kv_len_before_draft = self._get_kv_len(past_key_values) - K
